...
import { PDFDocumentProxy, PDFViewerParams, PDFPageProxy, PDFSource, PDFProgressData, PDFPromise, PDFTreeNode } from 'pdfjs-dist';
import { createEventBus } from './event-util';
...
if (typeof window !== 'undefined') {
  PDFJS = window['pdfjs-dist/build/pdf'] = require('pdfjs-dist/build/pdf');
...
@Component({
  ...
  template: `
    <div class="pdfViewerContainer" [hidden]="!isDocumentLoaded()"><div class="pdfViewer"></div></div>
  `,
  styles: [
    `
...
export class SimplePdfViewerComponent implements OnInit, OnDestroy {
   ...
   */
  public openDocument(src: string | Uint8Array | PDFSource, startAt: SimplePDFBookmark = SimplePDFBookmark.EMPTY_BOOKMARK): void {
  ...
  private initPDFJS() {
    ...
    this.pdfViewer._currentPageNumber = this.currentPage;
  }
  ...
  private loadFile() {
    ...
    if (this.src) {
      ...
      // loaded
      (<PDFPromise<PDFDocumentProxy>>progressSrc.promise).then(
        pdfDocument => {
          this.pdfViewer.setDocument(pdfDocument);
          this.pdfViewer.currentScaleValue = SimplePdfViewerComponent.PDF_VIEWER_DEFAULT_SCALE;
          this.pdfLinkService.setDocument(pdfDocument, null);
          this.pdf = pdfDocument;
          this.pdf.getOutline().then((nodes: PDFTreeNode[]) => {
  ...
  private mapOutline(nodes: PDFTreeNode[]): SimpleOutlineNode[] {
  ...
  private getScale(page: PDFPageProxy, priority: ScalePriority = ScalePriority.FULL): number {
  ...
  public getNumberOfMatches(): number {
    if (this.isDocumentLoaded()) {
      return this.pdfFindController._matchesCountTotal;
    }
    return 0;
  }
  ...
  public createBookmark(): Promise<SimplePDFBookmark> {
    ...
    const pagePromise = <Promise<PDFPageProxy>>(<any>this.pdf.getPage(this.currentPage));
    return pagePromise.then((page: PDFPageProxy) => {
      const viewport = page.getViewport(1, this.rotation);
      const container = this.getContainer();
